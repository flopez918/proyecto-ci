...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
			timeHtml = '<span class="fc-time">' + htmlEscape(this.getEventTimeText(event)) + '</label>';
		}
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						...
						'>' +
							'<span>' + htmlEscape(timeText) + '</label>' +
						'</div>' :
						''
						) +
					(event.title ?
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (viewText || customText) {
								...
								innerHtml = "<span class='ui-icon ui-icon-" + themeIcon + "'></label>";
							}
							else if (normalIcon && !options.theme) {
								innerHtml = "<span class='fc-icon fc-icon-" + normalIcon + "'></label>";
							}
							else {
